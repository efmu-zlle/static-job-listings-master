      ...
      return `<div class="card">
                    <!-- tarjeta izquierda -->
                        ...
                        ${jobs.etiquetas
                          .map(
                            (tag) =>
                              `<button data-info="${tag}">${tag}</button>`
                          )
                          .join("")}
                        </div>
                    </div>  
...
const addfilterSearch = (tagToFilter) => {
  ...
  if (!searchArray.includes(tagToFilter)) {
    ...
    button.setAttribute("data-name", tagToFilter);
...
const updateSearchFilter = () => {
  let filterSearchTags = jobsArray.filter((job) =>
    searchArray.every((tag) => job.etiquetas.includes(tag))
  );
...
window.addEventListener("click", (e) => {
  ...
  if (e.target.getAttribute("data-name")) {
    ...
    if (searchArray.includes(dataName)) {
      removeTagsSearch(dataName);
    }
  }
});
